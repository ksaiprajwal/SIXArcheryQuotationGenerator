...
def generate_pdf(doc):
    ...
    def line(x1, y1, x2, y2, weight=.55, dotted=False):
        c.saveState()
    ...
    def box(x, y, width, height, weight=.55):
        c.saveState()
        c.setStrokeColor(INK)
        c.setLineWidth(weight)
    ...
    note_height = _paragraph(note, RIGHT-LEFT, 9, True)[1] + 3*mm if note else 0
    ...
    for page_index, page_rows in enumerate(pages):
        ...
        if note:
            block(note, LEFT, 10*mm, RIGHT-LEFT, note_height-2*mm, 9, True)
        box(LEFT, top, RIGHT-LEFT, BOTTOM-top, .9)
        ...
        block(tagline, 36*mm, brand_top+12*mm, 109*mm, 11*mm, 11, True, TA_CENTER)
        ...
        if len(number) <= 8 and kind != 'Quotation':
            text(number, 179*mm, brand_top+8*mm, 18*mm, 13, True, color=RED, align='right')
        else:
            text(number, 152*mm, brand_top+17*mm, 45*mm, 12, True, color=RED)
        text('Date: '+date.fromisoformat(doc['date']).strftime('%d.%m.%Y'), 152*mm, brand_top+27*mm, 45*mm, 9)
        # Buyer area and faint writing rules retain the printed-pad structure.
        text('Buyer', 12*mm, buyer_top+3*mm, 16*mm, 9)
        offset = 8.5*mm
        while offset < buyer_height:
            line(29*mm, buyer_top+offset, 198*mm, buyer_top+offset, .3, True)
            offset += 6.5*mm
        ...
        line(28*mm, meta_top+10*mm, 87*mm, meta_top+10*mm, .3, True)
        ...
        line(157*mm, meta_top+10*mm, 198*mm, meta_top+10*mm, .3, True)
        text('Freight', 118*mm, meta_top+13*mm, 17*mm, 9)
        block(doc.get('freight', ''), 137*mm, meta_top+12*mm, 60*mm, 7*mm, 9)
        line(137*mm, meta_top+18*mm, 198*mm, meta_top+18*mm, .3, True)
        # Full-height ruled table, with separate rupees/paise columns.
        for x in xs[1:-1]:
            line(x, table_top if x != xs[-2] else table_top+4*mm, x, grid_bottom)
        text('S.No.', xs[0]+.5*mm, table_top+2.5*mm, 9*mm, 8, align='center')
        text('DESCRIPTION OF GOODS', xs[1]+2*mm, table_top+2.5*mm, 94*mm, 9, align='center')
        block('HSN\nCode', xs[2]+mm, table_top+.4*mm, 12*mm, 8*mm, 8, align=TA_CENTER)
        for label, a, b in [('Qty.', 3, 4), ('RATE', 4, 5)]:
            text(label, xs[a]+mm, table_top+2.5*mm, xs[b]-xs[a]-2*mm, 9, align='center')
        text('AMOUNT', xs[5], table_top+.4*mm, xs[-1]-xs[5], 8, align='center')
        text('Rs.', xs[5]+2*mm, table_top+4.5*mm, 23*mm, 8)
        text('Ps.', xs[6], table_top+4.5*mm, 8*mm, 8, align='center')
        y = grid_top
        for i, product, paragraph, height in page_rows:
            ...
            text(f"{product['quantity']:g}", xs[3]+mm, y+2*mm, 14*mm, 10, align='center')
            text(f"{money(product['price']):.2f}", xs[4]+mm, y+2*mm, 15*mm, 9, align='right')
            whole, fraction = f'{line_amounts[i]:.2f}'.split('.')
            text(whole, xs[5]+mm, y+2*mm, 25*mm, 10, align='right')
            text(fraction, xs[6]+.5*mm, y+2*mm, 7*mm, 10, align='center')
            y += height
            line(LEFT, y, RIGHT, y, .45)
        ...
        if last:
            ...
            for offset in [15, 24]:
                line(12*mm, grid_bottom+offset*mm, 134*mm, grid_bottom+offset*mm, .3, True)
            text('BANK DETAILS', 12*mm, grid_bottom+26*mm, 122*mm, 8, True, align='center')
            line(62*mm, grid_bottom+29.5*mm, 84*mm, grid_bottom+29.5*mm, .35)
        ...
        for index, (label, value) in enumerate(summary):
            ...
            text(label, footer_split+2*mm, sy+summary_h/2-5, 26*mm, 10, True)
            whole, fraction = f'{value:.2f}'.split('.')
            text(whole, 166*mm, sy+summary_h/2-5, 25*mm, 10, align='right')
            text(fraction, 192.5*mm, sy+summary_h/2-5, 7*mm, 10, align='center')
        ...
        if last:
            ...
            text('For '+company['name'], 138*mm, 269*mm, 60*mm, 8, True, align='center', color=RED)
            if doc.get('signed') and (ROOT/'signature.png').exists():
                c.drawImage(str(ROOT/'signature.png'), 154*mm, PAGE_H-282*mm,
                            width=31*mm, height=10*mm, preserveAspectRatio=True, anchor='c', mask='auto')
    ...
    if notes_appendix:
        ...
        count = 1
        while pending:
            text(f'{title} {number} - NOTES & TERMS', 16*mm, 15*mm, 178*mm, 12, True)
